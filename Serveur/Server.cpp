...
void Server::Work() {
    while (true) {
        if (_dataListVector.size() != 0) {

            //Check informations from the game move and send it to the other player
            json data = _dataListVector[0];
            ...
            data.clear();
            _dataListVector.erase(_dataListVector.begin());
        }
	} 
}
